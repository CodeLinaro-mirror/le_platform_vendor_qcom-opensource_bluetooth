...
static void bta_avk_st_rc_timer(tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data);
...
const tBTA_AVK_CO_FUNCTS bta_avk_a2d_cos =
{
...
const tBTA_AVK_SACT bta_avk_a2d_action[] =
{
    bta_avk_do_disc_a2d,     /* BTA_AVK_DO_DISC  */
...
static const UINT16 bta_avk_stream_evt_fail[] = {
...
void bta_avk_stream_data_cback(UINT8 handle, BT_HDR *p_pkt, UINT32 time_stamp, UINT8 m_pt);
static void bta_avk_stream0_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data);
static void bta_avk_stream1_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data);
#if BTA_AVK_NUM_STRS > 2
static void bta_avk_stream2_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data);
#endif
#if BTA_AVK_NUM_STRS > 3
static void bta_avk_stream3_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data);
#endif
#if BTA_AVK_NUM_STRS > 4
static void bta_avk_stream4_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data);
#endif
#if BTA_AVK_NUM_STRS > 5
static void bta_avk_stream5_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data);
#endif
/* the array of callback functions to receive events from AVDT control channel */
tAVDT_CTRL_CBACK * const bta_avk_dt_cback[] =
{
    bta_avk_stream0_cback
...
** Returns          void
***********************************************/
static UINT8 bta_avk_get_scb_handle(tBTA_AVK_SCB *p_scb, UINT8 local_sep)
{
    UINT8 xx =0;
    const int NON_A2DP = 0xFF;
    for (xx = 0; xx<BTA_AVK_MAX_SEPS; xx++)
    {
            ...
            (p_scb->seps[xx].codec_type == p_scb->codec_type))
        {
            if (p_scb->seps[xx].codec_type != NON_A2DP)
                ...
                UINT8 vendorId = p_scb->cfg.codec_info[BTA_AVK_VENDOR_ID_TYPE_IDX];
                UINT8 codecId = p_scb->cfg.codec_info[BTA_AVK_CODEC_ID_TYPE_IDX];
    ...
    APPL_TRACE_DEBUG(" bta_avk_get_scb_handle appropiate sep_type not found")
...
** Returns          void
***********************************************/
static UINT8 bta_avk_get_scb_sep_type(tBTA_AVK_SCB *p_scb, UINT8 tavdt_handle)
{
    UINT8 xx =0;
    for (xx = 0; xx<BTA_AVK_MAX_SEPS; xx++)
    {
    ...
    APPL_TRACE_DEBUG(" bta_avk_get_scb_sep_type appropiate handle not found")
...
**
*******************************************************************************/
static void bta_avk_save_addr(tBTA_AVK_SCB *p_scb, const BD_ADDR b)
{
    APPL_TRACE_DEBUG("bta_avk_save_addr r:%d, s:%d",
...
static void bta_avk_st_rc_timer(tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_st_rc_timer rc_handle:%d, use_rc: %d",
...
**
*******************************************************************************/
static BOOLEAN bta_avk_next_getcap(tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    int     i;
    ...
    for (i = p_scb->sep_info_idx; i < p_scb->num_seps; i++)
    {
            ...
            (p_scb->sep_info[i].media_type == p_scb->media_type))
        {
            ...
            (*p_req)(p_scb->peer_addr,
                           ...
                           p_scb->p_cap, bta_avk_dt_cback[p_scb->hdi]);
            sent_cmd = TRUE;
            break;
        }
    }
...
**
*******************************************************************************/
static void bta_avk_proc_stream_evt(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data, int index)
{
    tBTA_AVK_STR_MSG     *p_msg;
    UINT16              sec_len = 0;
    tBTA_AVK_SCB         *p_scb = bta_avk_cb.p_scb[index];
    int                 xx;
    APPL_TRACE_VERBOSE("bta_avk_proc_stream_evt on the index : %d", index);
    ...
    if (p_scb && (p_msg = (tBTA_AVK_STR_MSG *) GKI_getbuf((UINT16) (sizeof(tBTA_AVK_STR_MSG) + sec_len))) != NULL)
    {
        ...
        if (p_data != NULL)
        {
             ...
             * may be dereferenced there by passing argument 1 to function 'bta_avk_find_lcb' at line 500
             * added another null check for below condition to get rid of kw error */
            if (bd_addr != NULL && (bta_avk_find_lcb(bd_addr, BTA_AVK_LCB_FIND) != NULL) && (bta_avk_is_scb_init(p_scb)))
            {
                ...
                {
                    if ((bta_avk_cb.p_scb[xx]) && (xx != index))
                    {
...
/* false-positive: bta_avk_conn_cback only processes AVDT_CONNECT_IND_EVT and AVDT_DISCONNECT_IND_EVT event
 ...
 * by passing argument 2 to function 'bta_avk_conn_cback'. */
    if (p_data && bd_addr)
    {
...
/*******************************************************************************
**
** Function         bta_avk_stream_data_cback
**
...
void bta_avk_stream_data_cback(UINT8 handle, BT_HDR *p_pkt, UINT32 time_stamp, UINT8 m_pt)
{
    int index = 0;
    tBTA_AVK_SCB         *p_scb ;
    APPL_TRACE_DEBUG("bta_avk_stream_data_cback avdt_handle: %d pkt_len=0x%x  ofst = 0x%x", handle,p_pkt->len,p_pkt->offset);
...
static void bta_avk_stream0_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data)
{
    APPL_TRACE_VERBOSE("bta_avk_stream0_cback avdt_handle: %d event=0x%x", handle, event);
...
static void bta_avk_stream1_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data)
{
    APPL_TRACE_EVENT("bta_avk_stream1_cback avdt_handle: %d event=0x%x", handle, event);
...
static void bta_avk_stream2_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data)
{
    APPL_TRACE_EVENT("bta_avk_stream2_cback avdt_handle: %d event=0x%x", handle, event);
...
static void bta_avk_stream3_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data)
{
    APPL_TRACE_EVENT("bta_avk_stream3_cback avdt_handle: %d event=0x%x", handle, event);
...
static void bta_avk_stream4_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data)
{
    APPL_TRACE_EVENT("bta_avk_stream4_cback avdt_handle: %d event=0x%x", handle, event);
...
static void bta_avk_stream5_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data)
{
    APPL_TRACE_EVENT("bta_avk_stream5_cback avdt_handle: %d event=0x%x", handle, event);
...
/*******************************************************************************
**
** Function         bta_avk_a2d_sdp_cback
**
...
**
*******************************************************************************/
static void bta_avk_a2d_sdp_cback(BOOLEAN found, tA2D_Service *p_service)
{
    ...
    if ((p_msg = (tBTA_AVK_SDP_RES *) GKI_getbuf(sizeof(tBTA_AVK_SDP_RES))) != NULL)
    {
        ...
        p_scb = bta_avk_hndl_to_scb(bta_avk_cb.handle);
        if (p_scb)
        {
        ...
        {
            APPL_TRACE_ERROR ("bta_avk_a2d_sdp_cback, no scb found for handle(0x%x)", bta_avk_cb.handle);
        }
    }
}
...
**
*******************************************************************************/
static void bta_avk_adjust_seps_idx(tBTA_AVK_SCB *p_scb, UINT8 avdt_handle)
{
    int xx;
    const int NON_A2DP = 0xFF;
    APPL_TRACE_DEBUG("bta_avk_adjust_seps_idx codec_type: %d", p_scb->codec_type);
    for(xx=0; xx<BTA_AVK_MAX_SEPS; xx++)
    {
...
**
*******************************************************************************/
void bta_avk_switch_role (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    tBTA_AVK_RS_RES      switch_res = BTA_AVK_RS_NONE;
    tBTA_AVK_API_OPEN  *p_buf = &p_scb->q_info.open;
    UNUSED(p_data);

    APPL_TRACE_DEBUG("bta_avk_switch_role wait:x%x", p_scb->wait);
    ...
    p_scb->wait &= ~(BTA_AVK_WAIT_ROLE_SW_RES_OPEN|BTA_AVK_WAIT_ROLE_SW_RES_START);

    if (p_scb->q_tag == BTA_AVK_Q_TAG_OPEN)
    {
        if (bta_avk_switch_if_needed(p_scb) || !bta_avk_link_role_ok(p_scb, A2D_SET_MULTL_BIT))
        {
    ...
    if (switch_res != BTA_AVK_RS_NONE)
    {
        ...
        bta_avk_do_disc_a2d(p_scb, (tBTA_AVK_DATA *)p_buf);
    }
}
...
void bta_avk_role_res (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    tBTA_AVK_START   start;
    tBTA_AVK_OPEN    av_open;
    tBTA_AVK_ROLE_CHANGED role_changed;

    UINT8 cur_role = BTM_ROLE_UNDEFINED;

    APPL_TRACE_DEBUG("bta_avk_role_res q_tag:%d, wait:x%x, role:x%x", p_scb->q_tag, p_scb->wait, p_scb->role);
    ...
    else if (p_scb->q_tag == BTA_AVK_Q_TAG_OPEN)
    {
        if (p_scb->wait & BTA_AVK_WAIT_ROLE_SW_RES_OPEN)
        {
            ...
            if (p_data->role_res.hci_status != HCI_SUCCESS)
            {
                ...
                (*bta_avk_cb.p_cback)(BTA_AVK_OPEN_EVT, (tBTA_AVK *)&av_open);
            }
            else
            {
...
**
*******************************************************************************/
void bta_avk_delay_co (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
...
**
*******************************************************************************/
void bta_avk_do_disc_a2d (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_do_disc_a2d use_rc: %d rs:%d, oc:%d",
        p_data->api_open.use_rc, p_data->api_open.switch_res, bta_avk_cb.audio_open_cnt);
    ...
    case BTA_AVK_RS_NONE:
        if (bta_avk_switch_if_needed(p_scb) || !bta_avk_link_role_ok(p_scb, A2D_SET_MULTL_BIT))
        {
            ...
            memcpy(&p_scb->q_info.open, &p_data->api_open, sizeof(tBTA_AVK_API_OPEN));
            p_scb->wait |= BTA_AVK_WAIT_ROLE_SW_RES_OPEN;
    ...
    case BTA_AVK_RS_OK:
        ...
        if (bta_avk_link_role_ok(p_scb, A2D_SET_MULTL_BIT))
        {
            ok_continue = TRUE;
        }
        else
    ...
    if (p_scb->skip_sdp == TRUE)
    {
        ...
        bta_avk_a2d_sdp_cback(TRUE, &a2d_ser);
    }
    else
    {
        ...
        if(A2D_FindService(sdp_uuid, p_scb->peer_addr, &db_params,
                        bta_avk_a2d_sdp_cback) == A2D_SUCCESS)
        {
            return;
        }
        ...
        bta_avk_a2d_sdp_cback(FALSE, NULL);
    }
}
...
**
*******************************************************************************/
void bta_avk_cleanup(tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    tBTA_AVK_CONN_CHG msg;
    int             xx;
    UINT8           role = BTA_AVK_ROLE_AD_INT;
    UNUSED(p_data);
...
**
*******************************************************************************/
void bta_avk_free_sdb(tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);
    p_scb->sdp_discovery_started = FALSE;
}
...
void bta_avk_config_ind (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    p_scb->codec_type = p_evt_cfg->codec_info[BTA_AVK_CODEC_TYPE_IDX];
        ...
        (psc_mask != (p_scb->cfg.psc_mask&~AVDT_PSC_DELAY_RPT))) )
    {
        ...
        bta_avk_ssm_execute(p_scb, BTA_AVK_CI_SETCONFIG_FAIL_EVT, (tBTA_AVK_DATA *) &setconfig);
    }
    else
    {
        ...
        APPL_TRACE_DEBUG("bta_avk_config_ind: SEID: %d use_rc: %d cur_psc_mask:0x%x", p_info->seid, p_scb->use_rc, p_scb->cur_psc_mask);
...
**
*******************************************************************************/
void bta_avk_disconnect_req (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    tBTA_AVK_RCB *p_rcb;
    UNUSED(p_data);
...
void bta_avk_setconfig_rsp (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    int     i;
    UINT8   local_sep;
    tBTA_AVK_MEDIA av_sink_codec_info;

    /* we like this codec_type. find the sep_idx */
    local_sep = bta_avk_get_scb_sep_type(p_scb,avdt_handle);
    bta_avk_adjust_seps_idx(p_scb, avdt_handle);
    APPL_TRACE_DEBUG("bta_avk_setconfig_rsp: sep_idx: %d cur_psc_mask:0x%x", p_scb->sep_idx, p_scb->cur_psc_mask);
    ...
    if(p_data->ci_setconfig.err_code == AVDT_SUCCESS)
    {
        ...
        APPL_TRACE_DEBUG("bta_avk_setconfig_rsp recfg_needed:%d role:x%x num:%d",
...
void bta_avk_str_opened (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_str_opened l2c_cid: 0x%x stream_mtu: %d mtu: %d",
    ...
    bta_avk_cb.codec_type = p_scb->codec_type;
    {
        ...
        (*bta_avk_cb.p_cback)(BTA_AVK_OPEN_EVT, (tBTA_AVK *) &open);
        if(open.starting)
        {
...
/*******************************************************************************
**
** Function         bta_avk_co_audio_get_codec_type
**
** Description      Gets the p_scb->codec_type
**
** Returns          bta_avk_cb.codec_type
**
*******************************************************************************/
UINT8 bta_avk_co_audio_get_codec_type()
{
    APPL_TRACE_DEBUG("%s [bta_avk_cb.codec_type] %x", __func__, bta_avk_cb.codec_type);
    return bta_avk_cb.codec_type;
}
/*******************************************************************************
**
...
void bta_avk_security_ind (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    if (bta_avk_cb.features & BTA_AVK_FEAT_PROTECT)
    {
        ...
        */
        ...
        (*bta_avk_cb.p_cback)(BTA_AVK_PROTECT_REQ_EVT, (tBTA_AVK *) &protect_req);
    }
...
void bta_avk_security_cfm (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    if (bta_avk_cb.features & BTA_AVK_FEAT_PROTECT)
    {
        ...
        (*bta_avk_cb.p_cback)(BTA_AVK_PROTECT_RSP_EVT, (tBTA_AVK *) &protect_rsp);
    }
}
...
**
*******************************************************************************/
void bta_avk_do_close (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);
...
**
*******************************************************************************/
void bta_avk_connect_req (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UINT16 result;
    UNUSED(p_data);
...
void bta_avk_save_caps(tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_save_caps num_seps:%d sep_info_idx:%d wait:x%x",
        p_scb->num_seps, p_scb->sep_info_idx, p_scb->wait);
    memcpy(&cfg, p_scb->p_cap, sizeof(tAVDT_CFG));
    /* let application know the capability of the SNK */
    p_scb->p_cos->getcfg(p_scb->hndl, cfg.codec_info[BTA_AVK_CODEC_TYPE_IDX],
    ...
    if(p_scb->num_seps > p_scb->sep_info_idx)
    {
        ...
        /* request to AVDT.                                                     */
        if (!bta_avk_next_getcap(p_scb, p_data))
            getcap_done = TRUE;
    }
    else
        getcap_done = TRUE;
...
**
*******************************************************************************/
void bta_avk_set_use_rc (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);
...
**
*******************************************************************************/
void bta_avk_cco_close (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UINT16 mtu;
    UNUSED(p_data);
...
void bta_avk_open_failed (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_open_failed");
    ...
    for (idx = 0; (idx < BTA_AVK_NUM_STRS) && (is_av_opened == FALSE); idx++ )
    {
        ...
        if (p_opened_scb && (p_opened_scb->state == BTA_AVK_OPEN_SST) && (!bdcmp(p_opened_scb->peer_addr,p_scb->peer_addr )) )
            is_av_opened = TRUE;

    }

    /* if there is already an active AV connnection with the same bd_addr,
       don't send disconnect req, just report the open event with BTA_AVK_FAIL_GET_CAP status */
...
void bta_avk_getcap_results (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    media_type = p_scb->p_cap->codec_info[BTA_AVK_MEDIA_TYPE_IDX] >> 4;
    ...
    if ((p_scb->p_cap->num_codec != 0) &&
        ...
        (p_scb->p_cos->getcfg(p_scb->hndl, p_scb->p_cap->codec_info[BTA_AVK_CODEC_TYPE_IDX],
            cfg.codec_info, &p_scb->sep_info_idx, p_info->seid,
            &cfg.num_protect, cfg.protect_info) == 0))
    {
        ...
        p_scb->codec_type = cfg.codec_info[BTA_AVK_CODEC_TYPE_IDX];
...
void bta_avk_setconfig_rej (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    bta_avk_adjust_seps_idx(p_scb, avdt_handle);
    APPL_TRACE_DEBUG("bta_avk_setconfig_rej: sep_idx: %d",p_scb->sep_idx);
    AVDT_ConfigRsp(p_scb->avdt_handle, p_scb->avdt_label, AVDT_ERR_UNSUP_CFG, 0);

    bdcpy(reject.bd_addr, p_data->str_msg.bd_addr);
    reject.hndl = p_scb->hndl;
    (*bta_avk_cb.p_cback)(BTA_AVK_REJECT_EVT, (tBTA_AVK *) &reject);
}
...
**
*******************************************************************************/
void bta_avk_discover_req (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);

    /* send avdtp discover request */

    AVDT_DiscoverReq(p_scb->peer_addr, p_scb->sep_info, BTA_AVK_NUM_SEPS, bta_avk_dt_cback[p_scb->hdi]);
}
...
void bta_avk_do_start (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_do_start sco_occupied:%d, role:x%x, started:%d", bta_avk_cb.sco_occupied, p_scb->role, p_scb->started);
...
void bta_avk_str_stopped (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    if (p_data && p_data->api_stop.suspend)
    {
        ...
        if ((sus_evt)&&(p_scb->state != BTA_AVK_RCFG_SST))
        {
            suspend_rsp.status = BTA_AVK_SUCCESS;
            suspend_rsp.initiator = TRUE;
            (*bta_avk_cb.p_cback)(BTA_AVK_SUSPEND_EVT, (tBTA_AVK *) &suspend_rsp);
        }
    }
    else
    {
        suspend_rsp.status = BTA_AVK_SUCCESS;
        suspend_rsp.initiator = TRUE;
        APPL_TRACE_EVENT("bta_avk_str_stopped status %d", suspend_rsp.status);

        /* send STOP_EVT event only if not in reconfiguring state */
        if (p_scb->state != BTA_AVK_RCFG_SST)
        {
            (*bta_avk_cb.p_cback)(BTA_AVK_STOP_EVT, (tBTA_AVK *) &suspend_rsp);
        }
    }
}
...
void bta_avk_reconfig (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_reconfig r:%d, s:%d idx: %d (o:%d)",
        p_scb->recfg_sup, p_scb->suspend_sup,
...
**
*******************************************************************************/
void bta_avk_data_path (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    BT_HDR  *p_buf = NULL;
    UINT32  data_len;
    UINT32  timestamp;
...
void bta_avk_start_ok (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_start_ok wait:x%x, role:x%x", p_scb->wait, p_scb->role);
    ...
    if (p_scb->wait & (BTA_AVK_WAIT_ROLE_SW_RES_OPEN|BTA_AVK_WAIT_ROLE_SW_RES_START))
    {
    ...
    if(p_scb->media_type == AVDT_MEDIA_AUDIO)
    {
         ...
         * However, bta_avk_stream_chg is not called to increase bta_avk_cb.audio_count yet.
    ...
    else if ((new_role & BTA_AVK_ROLE_AD_ACP) && (new_role & BTA_AVK_ROLE_SUSPEND_OPT))
    {
...

    {
        ...
        APPL_TRACE_DEBUG("bta_avk_start_ok suspending: %d, role:x%x, init %d",
...
**
*******************************************************************************/
void bta_avk_start_failed (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);
...
**
*******************************************************************************/
void bta_avk_clr_cong (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);
...
void bta_avk_suspend_cfm (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG ("bta_avk_suspend_cfm:audio_open_cnt = %d, err_code = %d",
        bta_avk_cb.audio_open_cnt, err_code);
    ...
    if (p_scb->started == FALSE)
    {
        ...
        */
        APPL_TRACE_WARNING("bta_avk_suspend_cfm: already suspended, ignore, err_code %d",
                            err_code);
        return;
    }
    ...
    if (err_code && (err_code != AVDT_ERR_BAD_STATE))
    {
        ...
        APPL_TRACE_ERROR ("bta_avk_suspend_cfm: suspend failed, closing connection");
...
void bta_avk_rcfg_str_ok (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    p_scb->l2c_cid      = AVDT_GetL2CapChannel(p_scb->avdt_handle);
    APPL_TRACE_DEBUG("bta_avk_rcfg_str_ok: l2c_cid: %d", p_scb->l2c_cid);
...
void bta_avk_rcfg_failed (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_rcfg_failed num_recfg: %d, conn_lcb:0x%x",
        p_scb->num_recfg, bta_avk_cb.conn_lcb);
    if(p_scb->num_recfg > BTA_AVK_RECONFIG_RETRY)
    {
        bta_avk_cco_close(p_scb, p_data);
        /* report failure */
        evt.status = BTA_AVK_FAIL_STREAM;
        evt.chnl   = p_scb->chnl;
        evt.hndl   = p_scb->hndl;
        (*bta_avk_cb.p_cback)(BTA_AVK_RECONFIG_EVT, (tBTA_AVK *)&evt);
        /* go to closing state */
...
**
*******************************************************************************/
void bta_avk_rcfg_connect (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);

    p_scb->cong    = FALSE;
    p_scb->num_recfg++;
    APPL_TRACE_DEBUG("bta_avk_rcfg_connect num_recfg: %d", p_scb->num_recfg);
    if(p_scb->num_recfg > BTA_AVK_RECONFIG_RETRY)
    {
...
**
*******************************************************************************/
void bta_avk_rcfg_discntd (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    tBTA_AVK_RECONFIG    evt;
    UNUSED(p_data);

    APPL_TRACE_DEBUG("bta_avk_rcfg_discntd num_recfg: %d", p_scb->num_recfg);
    p_scb->num_recfg++;
    if(p_scb->num_recfg > BTA_AVK_RECONFIG_RETRY)
    {
        /* report failure */
...
void bta_avk_suspend_cont (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    {
        APPL_TRACE_DEBUG("bta_avk_suspend_cont calling AVDT_ReconfigReq");
        /* reconfig the stream */
...
void bta_avk_rcfg_cfm (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    /*
    APPL_TRACE_DEBUG("bta_avk_rcfg_cfm");
    */
    if (err_code)
    {
...
**
*******************************************************************************/
void bta_avk_rcfg_open (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);

    APPL_TRACE_DEBUG("bta_avk_rcfg_open, num_disc_snks = %d", p_scb->num_disc_snks);
    ...
    {
        p_scb->codec_type = p_scb->p_cap->codec_info[BTA_AVK_CODEC_TYPE_IDX];
...
**
*******************************************************************************/
void bta_avk_security_rej (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    UNUSED(p_data);
...
**                  This function needs to be kept very similar to bta_avk_chk_start
**
** Returns          void
**
*******************************************************************************/
void bta_avk_chk_2nd_start (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    tBTA_AVK_SCB *p_scbi;
    int i;
...
void bta_avk_open_rc (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_avk_open_rc use_rc: %d, wait: x%x role:x%x", p_scb->use_rc, p_scb->wait, p_scb->role);
    if ((p_scb->wait & BTA_AVK_WAIT_ROLE_SW_BITS) && (p_scb->q_tag == BTA_AVK_Q_TAG_START))
    {
    ...
    if(p_scb->use_rc == TRUE || (p_scb->role & BTA_AVK_ROLE_AD_ACP) )
    {
        ...
        else
        {
            ...
            bta_avk_rc_disc((UINT8)(p_scb->hdi + 1));
        }
    }
    else
    {
...
void bta_avk_open_at_inc (tBTA_AVK_SCB *p_scb, tBTA_AVK_DATA *p_data)
{
    ...
    else
    {
        ...
        APPL_TRACE_ERROR(" bta_avk_open_at_inc ReSetting collision mask  ");
        p_scb->coll_mask = 0;
        bta_avk_set_scb_sst_init (p_scb);
